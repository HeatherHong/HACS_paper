...
We sincerely thank the reviewers for the thoughtful and constructive feedback. We are encouraged that the reviewers find the problem formulation clear and recognize the practical relevance of HASC. Below we address all raised concerns with precise references to our manuscript and strengthen the methodological rigor of our presentation.
...
## 6. Sequence Length and Model Scale Sensitivity (New Reviewer)

**Reviewer concern:** While batch-level scaling is well analyzed, the scalability of the proposed ideas with respect to model size (sequence length, layer-wise weight parameter size, KV-cache size) is insufficient. The claim that heterogeneity is scale-invariant lacks supporting analysis or experiments for larger models. Additionally, the repeated phrase "on non-uniform hardware" at the end of the Abstract should be removed.

**Response:**

We sincerely thank the reviewer for this important observation. We clarify that while we cannot conduct additional experiments at this stage due to hardware constraints (our largest GPU has 24 GB memory, insufficient for 7B+ models without offloading), our theoretical analysis in Section IV.6 demonstrates that the performance benefits of HASC scale with heterogeneity ratios rather than absolute model size. Specifically, as model size increases, the available KV-cache headroom shrinks disproportionately on memory-constrained devices, which would amplify—rather than diminish—the benefits of our memory-aware capacity optimization. We have added a more explicit discussion of sequence length sensitivity to Section IV.6 and Section V.1 to address this concern directly.

**Sequence length analysis (newly added to Section IV.6):**

As $L_{\text{seq}}$ increases, KV-cache memory consumption grows linearly. Denoting the per-request KV-cache footprint as $M_{\text{kv}} = 2 \cdot n_{\text{layers}} \cdot n_{\text{heads}} \cdot d_{\text{head}} \cdot L_{\text{seq}} \cdot \text{sizeof(FP16)}$, the feasible batch size bound from Equation (7) becomes:

$$B_g \leq \frac{M_g - M_{\text{model}}}{m_{\text{token}} + M_{\text{kv}}}$$

As $L_{\text{seq}}$ increases, the denominator grows, reducing feasible batch size on all devices. Critically, this reduction is **asymmetric**: memory-constrained devices (e.g., TITAN X with 12 GB) exhaust their headroom significantly faster than memory-rich devices (e.g., A5000 with 24 GB), since the available headroom $(M_g - M_{\text{model}})$ is smaller to begin with. This widening disparity means that under uniform batch sizing, the system becomes increasingly bottlenecked by the weakest device as sequence length grows—precisely the condition under which HASC's per-device capacity optimization provides the **greatest benefit**. Similarly, longer sequences amplify activation tensor volume in collective operations, further motivating topology-aware hierarchical communication. Consequently, the performance advantages of HASC are expected to become **more pronounced**, not less, as sequence length increases toward production-scale workloads.

**Weight parameter size and KV-cache size analysis:**

As discussed in Section IV.6 (Equations 6–8), as model size increases:

- **Communication overhead** scales with hidden dimension $d_{\text{model}}$ rather than total parameter count. The relative bandwidth ratio $B_{\text{fast}}/B_{\text{slow}} \approx 128\times$ remains constant, so the benefit of topology-aware routing is governed by bandwidth ratio, not model size.

- **Memory headroom gap widens** as model footprint grows (e.g., ~140 GB for a 70B model in FP16), leaving less free memory on constrained devices and further amplifying the benefit of memory-aware per-device batch sizing.

- **Compute throughput ratio** (~2× between A5000 and TITAN X) is determined by hardware FLOPS and memory bandwidth, which remain constant regardless of model size, preserving the benefit of throughput-aware scheduling.

**Newly added Limitation item (Section V.1, item 6):**

We have explicitly acknowledged this limitation in the revised manuscript: "Our current evaluation fixes $L_{\text{max}} = 2048$ and evaluates models up to 3B parameters. For longer sequences or larger models, KV-cache memory pressure grows proportionally with $L_{\text{seq}}$, which would amplify the memory capacity gap across heterogeneous devices and further motivate per-device batch sizing. Evaluating HASC on 7B+ models with variable sequence lengths remains an important direction for future work."

**Abstract correction:**

The repeated phrase "on non-uniform hardware" at the end of the Abstract has been removed in the revised manuscript.

---

## 7. Sensitivity to Network Bandwidth
...
We thank the reviewers for the insightful comments, which have substantially improved the clarity and rigor of our presentation. The revised manuscript now includes:
...
6. **Sequence length and model scale sensitivity analysis** with theoretical derivation showing HASC's benefits amplify rather than diminish at longer sequence lengths and larger model scales, and explicit acknowledgment as future work (Page 10, Section IV.6 "Sequence Length Sensitivity" + Page 11, Section V.1 item 6)

7. **Dedicated network sensitivity analysis section** (Page 9, Section IV.5) with ablation decomposition, theoretical per-step latency analysis (Equation 5), and quantitative evidence that communication contributes only ~18% of total improvement

8. **Abstract correction** removing the duplicated phrase "on non-uniform hardware" (Page 1)
